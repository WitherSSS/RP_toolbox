export async function onRequestGet(context) {
    ...
    if (!PASSWORD_KV) return new Response(JSON.stringify([]), { status: 200, headers: { "Content-Type": "application/json" } });
...
export async function onRequestPost(context) {
    ...
    if (!UPLOAD_PASSWORD) return new Response(JSON.stringify({ error: "服务器未配置环境变量 UPLOAD_PASSWORD，拒绝任何修改操作以防篡改" }), { status: 403 });
    
    try {
        const body = await context.request.json();
        const { password, data } = body;
        
        if (password !== UPLOAD_PASSWORD) return new Response(JSON.stringify({ error: "管理员密码错误" }), { status: 401 });
