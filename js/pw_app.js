...
document.addEventListener('DOMContentLoaded', () => { initAlphaSidebar(); setupTouchListeners(); fetchData(); });
...
function scrollToSection(letter) { const el = document.getElementById(`section-${letter}`); if (el) el.scrollIntoView({ behavior: 'auto', block: 'start' }); }
async function fetchData() {
    try {
        ...
        document.getElementById('listContainer').innerHTML = `<div class="text-center py-16 text-red-400 text-sm">拉取云端数据失败 (或处于离线模式未缓存)</div>`;
    }
}
function getFirstLetter(str) {
    ...
    try {
        const pinyin = firstChar.localeCompare('阿', 'zh') < 0 ? '#' : firstChar.localeCompare('八', 'zh') < 0 ? 'A' : firstChar.localeCompare('擦', 'zh') < 0 ? 'B' : firstChar.localeCompare('搭', 'zh') < 0 ? 'C' : firstChar.localeCompare('蛾', 'zh') < 0 ? 'D' : firstChar.localeCompare('发', 'zh') < 0 ? 'E' : firstChar.localeCompare('噶', 'zh') < 0 ? 'F' : firstChar.localeCompare('哈', 'zh') < 0 ? 'G' : firstChar.localeCompare('击', 'zh') < 0 ? 'H' : firstChar.localeCompare('喀', 'zh') < 0 ? 'J' : firstChar.localeCompare('垃', 'zh') < 0 ? 'K' : firstChar.localeCompare('妈', 'zh') < 0 ? 'L' : firstChar.localeCompare('拿', 'zh') < 0 ? 'M' : firstChar.localeCompare('哦', 'zh') < 0 ? 'N' : firstChar.localeCompare('啪', 'zh') < 0 ? 'O' : firstChar.localeCompare('期', 'zh') < 0 ? 'P' : firstChar.localeCompare('然', 'zh') < 0 ? 'Q' : firstChar.localeCompare('撒', 'zh') < 0 ? 'R' : firstChar.localeCompare('塌', 'zh') < 0 ? 'S' : firstChar.localeCompare('挖', 'zh') < 0 ? 'T' : firstChar.localeCompare('昔', 'zh') < 0 ? 'W' : firstChar.localeCompare('压', 'zh') < 0 ? 'X' : firstChar.localeCompare('匝', 'zh') < 0 ? 'Y' : 'Z';
        return pinyin;
    } catch(e) { return '#'; }
}
...
function toggleAccordion(el) { const content = el.nextElementSibling; if (content.classList.contains('open')) { content.style.maxHeight = '0px'; content.classList.remove('open'); } else { content.style.maxHeight = content.scrollHeight + 'px'; content.classList.add('open'); } }
function handleSearch() { renderRecords(document.getElementById('searchInput').value); }
