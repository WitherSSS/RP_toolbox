const array = ["北京四方", "南瑞继保", "长园深瑞", "国电南自", "国电南自（1200）", "南瑞科技", "许继电气", "其它"];
const array_k = [
    [-30, 85, 90, -80, 80],
    [45, 90, 0, -75, 90],
    [-30, 85, 90, -80, 90],
...
export const initMTBP = () => {
    ...
    document.getElementById('direction').addEventListener('change', () => drawZoneChart());
    renderDynamicInputs();
};
...
function toggleZoneMode() {
    ...
    if (modeTextEl) modeTextEl.innerText = isOvercurrentMode ? "主变过流" : "主变零流";
    renderDynamicInputs();
...
function renderDynamicInputs() {
    ...
    if (!document.getElementById('results').classList.contains('hidden')) drawZoneChart();
}
...
function drawZoneChart() {
    ...
    const sensAngleRaw = parseFloat(document.getElementById('sensAngle').value) || 0;
    const opZone = parseFloat(document.getElementById('opZone').value) || 0;
    const wiringVal = isOvercurrentMode ? (parseFloat(document.getElementById('wiringMode').value) || 0) : 0;
    
    const baseCurrentAngle = isOvercurrentMode ? -(sensAngleRaw + wiringVal) : sensAngleRaw;
    const currentAngle = (baseCurrentAngle + (direction === 'busbar' ? 180 : 0)) % 360;
    
    let refVLabel = "", refVAngle = 0;
    if (isOvercurrentMode) {
        if (wiringVal === 0) { refVLabel = "U1"; refVAngle = 0; }
        else { refVLabel = "Ubc"; refVAngle = -90; }
    } else {
        refVLabel = "3U0"; refVAngle = -180;
    }
    ...
    const bound1 = currentAngle + opZone, bound2 = currentAngle - opZone;
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ...
    const drawDashedLine = (deg, label) => {
        ...
        if (displayDeg <= -180) displayDeg += 360;
        ctx.fillText(label + ' (' + displayDeg + '°)', getX(deg, radius + 15), getY(deg, radius + 15));
    };
    drawDashedLine(bound1, "上边界");
    drawDashedLine(bound2, "下边界");
    drawVector(ctx, cx, cy, refVAngle, radius * 0.7, '#3b82f6', refVLabel, getX, getY);
    drawVector(ctx, cx, cy, currentAngle, radius * 0.9, '#ef4444', currentLabel, getX, getY);
}

function drawVector(ctx, cx, cy, deg, length, color, label, getX, getY) {
    const endX = getX(deg, length), endY = getY(deg, length);
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ...
    if (displayDeg > 180) displayDeg -= 360;
    if (displayDeg <= -180) displayDeg += 360;
    ctx.fillText(`${label}: ${displayDeg}°`, getX(deg, length + 15), getY(deg, length + 15));
}