...
function renderSlopes(index) {
  ...
  if (index !== 2)
    html += `<div class="input-group"><label>三段斜率:</label><input type="number" step="any" name="K3" value="${k[2]}" onfocus="this.select()"  onblur="if(this.value==='') this.value='${k[2]}'"><span class="unit"></span></div>`;
  ...
  if (index !== 2)
    html += `<div class="input-group"><label>拐点2:</label><input type="number" step="any" name="Z2" value="${k[4]}" onfocus="this.select()"  onblur="if(this.value==='') this.value='${k[4]}'"><span class="unit">Ie</span></div>`;
...
function createTableHtml(title, highCur, lowCur, clock, method) {
  ...
  let hint = title.includes("平衡")
    ? "装置内差流应接近 0。"
    : !isReverse
      ? "临界点：稍微 <span class='text-blue-600 font-bold'>↓降低↓</span> 低侧电流动作， <span class='text-red-600 font-bold'>↑增加↑</span> 不动作。"
      : "临界点：稍微 <span class='text-red-600 font-bold'>↑增加↑</span> 低侧电流动作， <span class='text-blue-600 font-bold'>↓降低↓</span> 不动作。";
...
function calculate() {
  ...
  const rgzsInputs = document.querySelectorAll(
    '#rgzs-container input[name="RGZS"]',
  );
  let rgzsList = [];
  ...
  let html = "";
  if (!isManual && currentIndex !== 2)
    html += createTableHtml("1. 平衡校验", Ieh, Iel, Clock, TestMethod);
  ...
  if (!isManual) {
    ...
    const IhX2 = (
      ((2 * X2 + Iqd + K1 * Z1 + (X2 - Z1) * K2) * Ieh) /
      2
    ).toFixed(3);
    const IlX2 = (
      (((2 * X2 - Iqd - K1 * Z1 - (X2 - Z1) * K2) * Ieh) / 2) *
      y
    ).toFixed(3);
    html += createTableHtml(
      `3. ${currentIndex === 2 ? "" : "第二段"}校验 (Ir=${X2}Ie)`,
      IhX2,
      IlX2,
      Clock,
      TestMethod,
    ...
    if (currentIndex !== 2) {
      ...
      const IhX3 = (
        ((2 * X3 + Iqd + K1 * Z1 + (Z2 - Z1) * K2 + (X3 - Z2) * K3) * Ieh) /
        2
      ).toFixed(3);
      const IlX3 = (
        (((2 * X3 - Iqd - K1 * Z1 - (Z2 - Z1) * K2 - (X3 - Z2) * K3) * Ieh) /
    ...
    rgzsList.forEach((X, idx) => {
      ...
      if (X > Z1 && X <= Z2) {
        const IhX = (
          ((2 * X + Iqd + K1 * Z1 + (X - Z1) * K2) * Ieh) /
          2
        ).toFixed(3);
        const IlX = (
          (((2 * X - Iqd - K1 * Z1 - (X - Z1) * K2) * Ieh) / 2) *
          y
        ).toFixed(3);
        html += createTableHtml(
          `人工校验点 ${idx + 1} (${currentIndex === 2 ? "" : "第二段"}, Ir=${X}Ie)`,
          IhX,
          IlX,
          Clock,
          TestMethod,
      ...
      if (X > Z2 && currentIndex !== 2) {
        const IhX = (
          ((2 * X + Iqd + K1 * Z1 + (Z2 - Z1) * K2 + (X - Z2) * K3) * Ieh) /
          2
        ).toFixed(3);
        const IlX = (
          (((2 * X - Iqd - K1 * Z1 - (Z2 - Z1) * K2 - (X - Z2) * K3) * Ieh) /
  ...
  if (currentIndex === 2)
    html += `<div class="p-3 bg-orange-50 text-orange-700 text-xs rounded mb-4">高值比率差动不受CT断线闭锁...</div>`;
  if (!isManual && currentIndex !== 2)
    html += `<div class="result-card"><div class="result-card-title">5. 差速断校验 (高侧三相)</div><div class="text-sm space-y-1 p-2"><p>动作 (1.05倍): <span class="font-bold text-red-600">${IsdH} A</span></p><p>不动作 (0.95倍): <span class="font-bold text-green-600">${IsdL} A</span></p></div></div>`;

  document.getElementById("summaryInfo").innerHTML =
    `基准 Ie = ${Ieh} A | 系数 K = ${K_val}`;
...
function calculateReverse() {
  ...
  if (Math.abs(p2.Ir - p1.Ir) < 0.0001) {
    ...
    if (s1 !== s2)
      warnHtml += `<div class="mt-2 text-red-600 font-bold bg-red-50 p-2 rounded border border-red-200 text-xs">⚠️ 注意：输入的两点不在同一斜率段上！<br><span class="font-normal text-red-500">点1位于第 ${s1} 段 (Ir=${p1.Ir.toFixed(3)})，点2位于第 ${s2} 段 (Ir=${p2.Ir.toFixed(3)})。计算出的特性曲线可能失真。</span></div>`;
    if (p1.Id > calcContext.Isd || p2.Id > calcContext.Isd)
      warnHtml += `<div class="mt-2 text-red-600 font-bold bg-red-50 p-2 rounded border border-red-200 text-xs">⚠️ 警告：换算出的差动电流已大于速断定值 (${calcContext.Isd} Ie)！<br><span class="font-normal text-red-500">点1 Id = ${p1.Id.toFixed(3)} Ie | 点2 Id = ${p2.Id.toFixed(3)} Ie。<br>请检查保护装置是否已动作于【差动速断】，导致比率差动判定失效！</span></div>`;
