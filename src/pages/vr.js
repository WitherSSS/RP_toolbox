...
class Complex {
  ...
  get angle() {
    return ((Math.atan2(this.imag, this.real) * 180) / Math.PI + 360) % 360;
  }
  ...
  afterDraw: (chart) => {
    ...
    chart.data.datasets.forEach((dataset, i) => {
      ...
      dataset.data.forEach((pt, idx) => {
        ...
        ctx.fillText(
          pt.label,
          targetX + Math.cos(angle) * 15 - 5,
          targetY - Math.sin(angle) * 15 + 5,
        );
      });
    });
  },
...
function updatePhasorChart(va, vb, vc, vp, vn, vz) {
  ...
  const limit =
    Math.max(va.abs, vb.abs, vc.abs, vp.abs, vn.abs, vz3.abs, 1) * 1.3;
    ...
    data: {
      datasets: [
        ...
        },
        {
          ...
          data: [
            toPt(vp, "pA"),
            toPt(vp.multiply(a2_op), "pB"),
            toPt(vp.multiply(a_op), "pC"),
          ],
          borderColor: "#eab308",
          borderDash: [5, 5],
          borderWidth: 1.5,
        ...
        },
        {
          ...
          data: [
            toPt(vn, "nA"),
            toPt(vn.multiply(a_op), "nB"),
            toPt(vn.multiply(a2_op), "nC"),
          ],
          borderColor: "#22c55e",
          borderDash: [5, 5],
          borderWidth: 1.5,
